...
#ifndef _rx_xlslice_1_2_core_h_
#define _rx_xlslice_1_2_core_h_

#include <systemc.h>
#include "properties.h"


class rx_xlslice_1_2_core : public sc_module 
{
    public: 
       rx_xlslice_1_2_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(rx_xlslice_1_2_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~rx_xlslice_1_2_core() = default;
